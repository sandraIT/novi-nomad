...
if ( !function_exists( 'unitravel_wp_scripts' ) ) {
	...
	function unitravel_wp_scripts() {
		...
		wp_enqueue_style( 'unitravel-fontello',  unitravel_get_file_url('css/fontello/css/fontello-embedded.css') );
...
if ( !function_exists('unitravel_theme_fonts_links') ) {
	function unitravel_theme_fonts_links() {
		...
		*/
		$google_fonts_enabled = ( 'off' !== _x( 'on', 'Google fonts: on or off', 'unitravel' ) );
		$custom_fonts_enabled = ( 'off' !== _x( 'on', 'Custom fonts (included in the theme): on or off', 'unitravel' ) );
		...
		if ( ($google_fonts_enabled || $custom_fonts_enabled) && !unitravel_storage_empty('load_fonts') ) {
			...
			if (count($load_fonts) > 0) {
				...
				if ($google_fonts && $google_fonts_enabled) {
					$links['google_fonts'] = sprintf('%s://fonts.googleapis.com/css?family=%s&subset=%s', unitravel_get_protocol(), $google_fonts, unitravel_get_theme_option('load_fonts_subset'));
				}
			}
		}
		return $links;
...
if ( !function_exists( 'unitravel_importer_set_options' ) ) {
	...
	function unitravel_importer_set_options($options=array()) {
		if (is_array($options)) {
			...
			// 		$options['files']['dark_demo']['title'] = esc_html__('Dark Demo', 'unitravel');
		}
		return $options;
	}
}
