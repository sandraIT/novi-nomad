...
if (!function_exists('unitravel_get_blog_title')) {
	function unitravel_get_blog_title() {
		...
		if (is_front_page())
			...
			$title = sprintf( esc_html__( '%s', 'unitravel' ), single_cat_title( '', false ) );
		else if ( is_tag() )
			$title = sprintf( esc_html__( 'Tag: %s', 'unitravel' ), single_tag_title( '', false ) );
		else if ( is_tax() )
			$title = sprintf( esc_html__( '%s', 'unitravel' ), single_term_title( '', false ) );
	...
	function unitravel_get_post_terms($delimiter=', ', $id=false, $taxonomy='category') {
		...
		if ( !empty( $terms ) ) {
			foreach( $terms as $term )
				$output .= ($output ? $delimiter : '') . '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '" title="' . sprintf( esc_attr__( 'View all posts in %s', 'unitravel' ), $term->name ) . '">' . esc_html( $term->name ) . '</a>';
		}
		return $output;
	}
}
