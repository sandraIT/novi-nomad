	...
	function unitravel_show_post_featured($args=array()) {
		...
		if ( $args['singular'] ) {
			...
			if ($has_thumb || !empty($args['show_no_image']) || (!$args['thumb_only'] && in_array($post_format, array('gallery', 'image', 'audio', 'video')))) {
				...
				} else if (!empty($args['show_no_image']) && ($image=unitravel_get_no_image())!='') {
				...
				if ($post_format == 'video' && !$args['thumb_only']) {
					$video = unitravel_get_post_video('', false);
					if (empty($video))
						$video = unitravel_get_post_iframe('', false);
					if (!empty($video)) {
						if ( $has_thumb ) {
				...
				if ($post_format == 'audio' && !$args['thumb_only']) {
					$audio = unitravel_get_post_audio('', false);
					if (empty($audio))
						$audio = unitravel_get_post_iframe('', false);
					if (!empty($audio)) {
						//Show metadata (for the future version)
						if (false && function_exists('wp_read_audio_metadata')) {
							$src = unitravel_get_post_audio($audio);
