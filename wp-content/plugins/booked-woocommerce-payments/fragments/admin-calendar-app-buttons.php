...
<?php else:
	...
	echo '<span class="booked-wc_status-text paid">';
		...
		if (current_user_can('manage_booked_options') && $appointment->order_id != 'manual') :
			echo '<a target="_blank" href="' . admin_url('/post.php?post=' . $appointment->order_id . '&action=edit') . '"><i class="booked-icon booked-icon-pencil"></i>&nbsp;&nbsp;' . __('Paid', 'booked-woocommerce-payments') . '</a>';
		else :
