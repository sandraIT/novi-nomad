...
if ( !function_exists( 'trx_addons_sc_layouts_title' ) ) {
	function trx_addons_sc_layouts_title($atts, $content=null){	
		...
		ob_start();
		set_query_var('trx_addons_args_sc_layouts_title', $atts);
		if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/title/tpl.'.trx_addons_esc($atts['type']).'.php')) != '') { include $fdir; }
		else if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/title/tpl.default.php')) != '') { include $fdir; }
				...
				"params" => array_merge(
					array(
						...
						array(
							...
							"std" => "0",
							"value" => array(esc_html__("Show post meta", 'trx_addons') => "1" ),
							"type" => "checkbox"
						),
						array(
							"param_name" => "title",
							...
							'edit_field_class' => 'vc_col-sm-4',
							"std" => "0",
							"value" => array(esc_html__("Show post title", 'trx_addons') => "1" ),
							"type" => "checkbox"
						),
						array(
							...
							'edit_field_class' => 'vc_col-sm-4',
							"std" => "0",
							"value" => array(esc_html__("Show breadcrumbs", 'trx_addons') => "1" ),
							"type" => "checkbox"
						)
					),
