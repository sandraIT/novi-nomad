...
if ( !function_exists( 'trx_addons_sc_layouts_language' ) ) {
	function trx_addons_sc_layouts_language($atts, $content=null){	
		...
		ob_start();
		set_query_var('trx_addons_args_sc_layouts_language', $atts);
		if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/language/tpl.'.trx_addons_esc($atts['type']).'.php')) != '') { include $fdir; }
		else if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/language/tpl.default.php')) != '') { include $fdir; }
