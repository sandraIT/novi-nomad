...
if ( !function_exists( 'trx_addons_sc_layouts_iconed_text' ) ) {
	function trx_addons_sc_layouts_iconed_text($atts, $content=null){	
		...
		ob_start();
		set_query_var('trx_addons_args_sc_layouts_iconed_text', $atts);
		if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/iconed_text/tpl.'.trx_addons_esc($atts['type']).'.php')) != '') { include $fdir; }
		else if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/iconed_text/tpl.default.php')) != '') { include $fdir; }
