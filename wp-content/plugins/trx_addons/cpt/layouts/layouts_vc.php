	...
	function trx_addons_cpt_layouts_vc_init() {
		...
		$param = array(
			...
			"value" => array(
				...
				esc_html__('Bottom panel', 'trx_addons') => 'bottom_panel',
				esc_html__('Header 1', 'trx_addons') => 'header_1'
			),
			"type" => "dropdown"
		);
	...
	function trx_addons_cpt_layouts_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_button'))) {
			if (!empty($atts['hide_on_tablet']) && !trx_addons_is_inherit($atts['hide_on_tablet']))
				$output = str_replace('class="sc_button ', 'class="hide_on_tablet sc_button ', $output);
			if (!empty($atts['hide_on_mobile']) && !trx_addons_is_inherit($atts['hide_on_mobile']))
				$output = str_replace('class="sc_button ', 'class="hide_on_mobile sc_button ', $output);
		} else if (in_array($sc, array('trx_sc_socials'))) {
			if (!empty($atts['hide_on_tablet']) && !trx_addons_is_inherit($atts['hide_on_tablet']))
				$output = str_replace('class="sc_socials ', 'class="hide_on_tablet sc_socials ', $output);
			if (!empty($atts['hide_on_mobile']) && !trx_addons_is_inherit($atts['hide_on_mobile']))
				$output = str_replace('class="sc_socials ', 'class="hide_on_mobile sc_socials ', $output);
		}
		return $output;
	}
}
