...
while ( have_posts() ) { the_post();
	...
	?>
    <article id="post-<?php the_ID(); ?>" <?php post_class( 'sport_single player_single itemscope' ); ?>
				...
				if (!empty($meta['birthday'])) {
					?><div class="sport_page_meta_item sport_page_meta_date">
						...
						<span class="sport_page_meta_item_value"><?php
							echo date(get_option('date_format'), strtotime($meta['birthday']));
						?></span>
					</div><?php
				}
				// Club/Country
						...
						?><li><?php
								...
								?><span class="sport_page_match_time"><a href="<?php echo esc_url($link); ?>"><?php 
									echo esc_html(!empty($meta['time_start']) 
													? date(get_option('time_format'), strtotime($meta['time_start'])) 
													: '--:--'); 
								?></a></span><?php
