...
while ( have_posts() ) { the_post();
	...
	?>
    <article id="post-<?php the_ID(); ?>" <?php post_class( 'sport_single competition_single itemscope' ); ?>
		...
		<section class="sport_page_header">	
			...
			if (!trx_addons_sc_layouts_showed('title')) {
				?>
				<h2 class="sport_page_title"><?php the_title(); ?></h2>
						...
						foreach ($matches as $id_match=>$name_match) {
							...
							if ($last_date != $meta['date_start']) {
								...
								?><li><a href="<?php echo esc_url(get_permalink($id)); ?>" class="sport_page_list_link"><?php
									?><span class="sport_page_round_time" data-round=".sport_page_list_round_<?php echo esc_html($id); ?>"></span><?php
									?><span class="sport_page_round_name"><?php echo esc_html($name).'. '.date(get_option('date_format'), strtotime($meta['date_start'])); ?></span><?php
								?></li></a><?php
							}
							?><li class="sport_page_list_round_<?php echo esc_html($id); ?>"><a href="<?php echo esc_url(get_permalink($id_match)); ?>" class="sport_page_list_link"><?php
								// Start time
