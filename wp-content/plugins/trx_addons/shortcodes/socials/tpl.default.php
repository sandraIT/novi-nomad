		...
		if ($socials_default) {
			...
			foreach ($args['icons'] as $item) {
				...
				if (!empty($icon) && !empty($item['link'])) {
					?><span class="social_item<?php if (!empty($item['with_caption']) && $item['with_caption'] == 1) echo ' '.esc_attr('social_item_with_caption'); ?>"><a href="<?php echo esc_url($item['link']); ?>" target="_blank" class="social_icons"><span class="<?php echo esc_attr($icon); ?>"></span><span class="social_item_caption"><?php if (((!empty($item['with_caption']) && $item['with_caption'] == 1)) && !empty($item['caption'])) trx_addons_show_layout($item['caption']); ?></span> </a></span><?php
				}
			}
		}
