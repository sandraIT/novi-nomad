			...
			?><select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'trx_addons' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo $i; ?>][]">
				...
				if ( $all_terms ) {
					foreach ( $all_terms as $term ) {
                        echo '<option value="' . esc_attr(version_compare(WOOCOMMERCE_VERSION, '3.0', '<') ? $term->term_slug : $term->term_id) . '" ' . selected( has_term( absint( $term->term_id ), wc_attribute_taxonomy_name($attribute_taxonomy->attribute_name), $thepostid ), true, false ) . '>' . esc_attr( apply_filters( 'woocommerce_product_attribute_term_name', $term->name, $term ) ) . '</option>';
					}
				}
			?></select>
